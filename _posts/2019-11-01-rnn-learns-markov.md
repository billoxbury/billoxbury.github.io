...
Let’s see what we can learn by fitting a recurrent network of LSTMs.
...
This last example has the most confusion among HMM states, and that is reflected in the 3-dimensional plot. Nevertheless, if we do a dimensional reduction to the plane using ttt-SNE for this example, we see that the separation is in fact still pretty good:
